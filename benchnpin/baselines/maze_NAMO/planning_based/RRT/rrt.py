...
class RRTPlanner:
    ...
    def plan(
        ...
        s = scene.snap_to_free(start) if snap_to_free else start
        g = scene.snap_to_free(goal) if snap_to_free else goal

        # Pass 1: boxes block
        path = self._run_rrt(s, g, scene, boxes_blocking=True)
        if path is not None:
        ...
        if ignore_boxes_fallback:
            path2 = self._run_rrt(s, g, scene, boxes_blocking=False)
    ...
    def plot_env(self, scene: _Scene,nodes,parent,path, ax=None):
        ...
        if scene.walls_infl:
            x, y = scene.walls_infl.exterior.xy
            ax.fill(x, y, color='lightgray', alpha=0.7, label='Inflated Walls')
            #holes
            for interior in scene.walls_infl.interiors:
                x, y = interior.xy
