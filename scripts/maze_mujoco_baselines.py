...
def main(cfg, job_id):
    ...
    if cfg.train.train_mode:
        ...
        if cfg.train.job_type == 'ppo':
            # ================================ PPO Policy =================================    
            ppo_policy = MazeMujocoPPO(model_name=model_name, cfg=cfg)
            ppo_policy.train(total_timesteps=cfg.train.total_timesteps, checkpoint_freq=cfg.train.checkpoint_freq, pretrained_checkpoint_path=cfg.train.pretrained_checkpoint_path)

        elif cfg.train.job_type == 'sac':
            # ================================ SAC Policy =================================
            sac_policy = MazeMujocoSAC(model_name=model_name, cfg=cfg)
            sac_policy.train(total_timesteps=cfg.train.total_timesteps, checkpoint_freq=cfg.train.checkpoint_freq, pretrained_checkpoint_path=cfg.train.pretrained_checkpoint_path)
    ...
    if cfg.evaluate.eval_mode:
        ...
        for policy_type, model_name in zip(cfg.evaluate.policy_types, cfg.evaluate.model_names):
            ...
            if policy_type == 'ppo':
                # ================================ PPO Policy =================================    
                ppo_policy = MazeMujocoPPO(model_name=model_name, model_path=model_path, cfg=cfg)
